...
from XML import generate_xml  # Importe a função generate_xml do seu módulo XML
...
def generate_lote_ne():
    global data_geracao_entry, sequencial_geracao_entry, ano_referencia_entry, ug_responsavel_entry, cpf_responsavel_entry, output_directory
    ...
    generate_xml(data_geracao, sequencial_geracao, ano_referencia, ug_responsavel, cpf_responsavel, output_directory)

    # Exiba um popup com a mensagem de sucesso
    messagebox.showinfo("Sucesso", "Lote Gerado com Sucesso")
    
root = tk.Tk()
root.title("Gerar Lote de NE")
...
sequencial_geracao_label = ttk.Label(form_frame, text="Número de Sequência:")
...
ano_referencia_label.grid(row=2, column=0, sticky="w")
ano_referencia_entry = ttk.Entry(form_frame)
ano_referencia_entry.grid(row=2, column=1, padx=(0,280), pady=5)

ug_responsavel_label = ttk.Label(form_frame, text="UG Responsável:")
ug_responsavel_label.grid(row=3, column=0, sticky="w")
ug_responsavel_entry = ttk.Entry(form_frame)
ug_responsavel_entry.grid(row=3, column=1, padx=(0,280), pady=5)

cpf_responsavel_label = ttk.Label(form_frame, text="CPF Responsável:")
cpf_responsavel_label.grid(row=4, column=0, sticky="w")
cpf_responsavel_entry = ttk.Entry(form_frame)
cpf_responsavel_entry.grid(row=4, column=1, padx=(0,280), pady=5)
...
output_path_label.grid(row=5, column=0, sticky="w")
output_path_entry = ttk.Entry(form_frame, width=50)
output_path_entry.grid(row=5, column=1, columnspan=5, padx=(0, 100), pady=5)

choose_output_button = ttk.Button(form_frame, text="Escolher Caminho", command=choose_output_path)
choose_output_button.grid(row=6, column=1, padx=(0,270), pady=5)# Ainda pode ajustar o pady como preferir

generate_button = ttk.Button(root, text="Gerar Lote de NE", command=generate_lote_ne)  # Remova os parênteses
generate_button.grid(row=1, column=0, padx=(0, 100), pady=10)
