...
def preencher_colunas(caminho_arquivo_copia):
    try:
        ...
        df['Fonte 4 Digitos'] = df['Fonte'].astype(str).str[:4]  # 4 primeiros dígitos
        df['Fonte 6 Digitos'] = df['Fonte'].astype(str).str[-6:]  # 6 últimos dígitos
        df['Natureza Despesa 2 Digitos'] = df['Natureza Despesa'].astype(str).str[:2]  # 2 primeiros dígitos
        df['Natureza Despesa 4 Digitos'] = df['Natureza Despesa'].astype(str).str[-4:]
        df['Natureza Despesa Centro'] = df['Natureza Despesa'].astype(str).str[2:4]
...
def generate_macro_vigencia(output_directory, df):
    ...
    for index, row in df.iterrows():
        ...
        fonte = str(row['Fonte'])
        pi = str(row['PI'])
        nd2 = str(row['Natureza Despesa 2 Digitos'])
        nd4 = str(row['Natureza Despesa 4 Digitos'])
        nd3 = str(row['Natureza Despesa Centro'])
        valor = str(row['Valor'])
        fontequatro = str(row['Fonte 4 Digitos'])
    ...
    <screen name="Tela{tela}" entryscreen="true" exitscreen="false" transient="false">
        ...
        <actions>
            <input value="31Dez251[tab]1527341[tab]{esfera}{ptres}{fontequatro}{nd2}1[tab]31Dez259999[tab]{observacao}[tab][tab][tab]R{fonteseis}{nd4}[tab][tab]{pi}{valor}[tab]A{fonteseis}{nd3}00[tab][tab][tab]{valor}[enter]" row="0" col="0" movecursor="true" xlatehostkeys="true" encrypted="false" />
        </actions>
        <nextscreens timeout="0" >
    ...
    # Salva o arquivo
    output_filename = os.path.join(output_directory, "Retirada de detalhamento.MAC")
...
def main():
    caminho_arquivo_original = r'W:\B - TED\7 - AUTOMAÇÃO\Detalhamento de credito\DETAORC dados.xlsx'
    caminho_arquivo_copia = r'W:\B - TED\7 - AUTOMAÇÃO\Detalhamento de credito\COPIA DETAORC dados.xlsx'
